...
def get_gemini_response(job_description, pdf_text, prompt):
    try:
...
st.title("\U0001F916 AI Powered Resume Screener for Acurrate Hiring")
    ...
    if not uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            if option == "Resume Summary":
                response = get_gemini_response("", cleaned_text, "Provide a concise summary of the resume, highlighting key skills and experience.")
                st.subheader(f"Summary")
                st.write(response)

            elif option == "Percentage Match with Job Description":
                response = get_gemini_response(clean_text(job_description), cleaned_text, "How well does this resume match the job description? Return only the percentage and a short explanation.")
                ...
                response = get_gemini_response("", cleaned_text, "Based on the resume content, suggest relevant job titles or industries.")
                st.subheader(f"Job Recommendations")
                st.write(response)

            elif option == "Skill Improvement Suggestions":
                response = get_gemini_response(clean_text(job_description), cleaned_text, "List key skills from the resume, compare with job description, and suggest additional skills to improve job alignment.")
                st.subheader(f"Skill Suggestions")
                st.write(response)
